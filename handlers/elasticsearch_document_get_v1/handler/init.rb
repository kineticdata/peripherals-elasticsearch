...
class ElasticsearchDocumentGetV1
  ...
  def execute
    ...
    id                    = @parameters["id"]
    include_source_fields = @parameters["include_source_fields"]
    exclude_source_fields = @parameters["exclude_source_fields"]
    error_handling        = @parameters["error_handling"]

    api_route = "#{api_server.chomp("/")}/#{index}/#{type}/#{id}"
    
    if include_source_fields.to_s.strip.empty? == false || include_source_fields.to_s.strip.empty? == false then
      api_route << "?"
    end
    
    api_route << "_source_include=#{URI.encode(include_source_fields)}&" if include_source_fields.to_s.strip.empty? == false
    api_route << "_source_exclude=#{URI.encode(exclude_source_fields)}" if exclude_source_fields.to_s.strip.empty? == false
    api_route.chomp("&");
    ...
    <results>
      ...
      <result name="Document">#{escape(response.body)}</result>
    </results>
    RESULTS
      ...
      if error_handling == "Raise Error"
        ...
        <<-RESULTS
        <results>
          <result name="Handler Error Message">#{error.http_code}: #{escape(error.response)}</result>
          <result name="Document"></result>
        </results>
        RESULTS
      end
  end
